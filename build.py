    ...
    for build_dir in dirs:
        ...
        if build_path.exists():
            ...
            for file_path in build_path.iterdir():
                if (
                    file_path.is_file()
                    and file_path.suffix not in exclude_extensions
                    and file_path.name not in ["base_library.zip", "python312.dll"]
                    and not file_path.name.startswith("_")
                    and (os.access(file_path, os.X_OK) or file_path.suffix == "")
                ):
                ...
                if subdir.is_dir():
                    for file_path in subdir.iterdir():
                        if (
                            ...
                            and file_path.suffix not in exclude_extensions
                            and file_path.name
                            not in ["base_library.zip", "python312.dll"]
                            and not file_path.name.startswith("_")
                            and (
...
def main():
    ...
    args = [
        ...
        "--mingw64",
        "--quiet",
        "--windows-company-name=ASLant",
        "--windows-file-version=1.0.0",
        "--output-dir=build",
        "--output-filename=PyBuilder",
